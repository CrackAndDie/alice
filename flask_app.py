                ...
                if not sessionStorage[user_id]['game_started']:
                    if 'да' in req['request']['nlu']['tokens']:
                        if len(sessionStorage[user_id]['guessed_cities']) == 3:
                            res['response']['text'] = f'{sessionStorage[user_id]["first_name_up"]}, все города угаданы, спасибо за игру!'
                        ...
                        res['response']['text'] = f'До свидания, {sessionStorage[user_id]["first_name_up"]}!'
                        res['end_session'] = True
                    else:
                        res['response']['text'] = f'{sessionStorage[user_id]["first_name_up"]}, не поняла ответа. Да или нет?'
                        res['response']['buttons'] = [
                            {
                                'title': 'да',
                                'hide': True
...
def play_game(res, req):
    ...
    if step == 1:
        ...
        res['response']['card']['title'] = f'{sessionStorage[user_id]["first_name_up"]}, какой это город?'
        ...
        if get_city(req) == city:
            res['response']['text'] = f'Правильно. {sessionStorage[user_id]["first_name_up"]}, a в какой стране этот город?'
            ...
            if attempt == 3:
                res['response']['text'] = f'{sessionStorage[user_id]["first_name_up"]}, Вы пытались. Это {city.title()}. А в какой стране этот город?'
                sessionStorage[user_id]['game_started'] = True
                sessionStorage[user_id]['guessed_cities'].append(city)
                if res['response'].get('buttons', 'gg') != 'gg':
                    res['response']['buttons'].append({
                ...
                res['response']['card'] = {}
                res['response']['card']['type'] = 'BigImage'
                res['response']['card']['title'] = f'{sessionStorage[user_id]["first_name_up"]}, не верно, попробуйте еще раз'
                res['response']['card']['image_id'] = cities[city][attempt - 1]
                res['response']['text'] = 'Я угадал!'
    elif step == 3:
        city = sessionStorage[user_id]['city']
        ...
        if get_count(req) == count:
            res['response']['text'] = f'Правильно. {sessionStorage[user_id]["first_name_up"]}, cыграем еще?'
            ...
            if attempt == 3:
                res['response']['text'] = f'{sessionStorage[user_id]["first_name_up"]}, Вы пытались. Это {count}. Сыграем еще?'
                sessionStorage[user_id]['game_started'] = False
                if res['response'].get('buttons', 'gg') != 'gg':
                    res['response']['buttons'].append({
                        "title": "Покажи город на карте",
                ...
                res['response']['text'] = f'Неверно! {sessionStorage[user_id]["first_name_up"]}, попробуйте еще раз'
    sessionStorage[user_id]['attempt'] += 1
